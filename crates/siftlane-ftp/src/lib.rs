    ...
    pub async fn connect(options: FtpConnectOptions) -> Result<Self, AppError> {
        ...
        let session = match options.security {
            FtpSecurity::Plain => {
                ...
                let password = options.password.expose_secret().to_string();
                stream
            ...
            FtpSecurity::ExplicitTls => {
                let stream = timeout(
                    ...
                    AsyncNativeTlsFtpStream::connect(address),
                )
                .await
                .map_err(|_| timed_out())
                .and_then(|result| result.map_err(map_ftp_error))?;
                let connector =
                    AsyncNativeTlsConnector::from(async_native_tls::TlsConnector::new());
...
impl RemoteFilesystem for FtpClient {
    ...
    async fn list_directory(&self, path: &str) -> Result<Vec<FileEntry>, AppError> {
        self.with_session(async |session| {
            ...
            let mut entries: Vec<FileEntry> = match lines {
                Ok(lines) => lines
                    .into_iter()
                    .filter_map(|line| parse_mlsx(&line, path))
                    .collect(),
                Err(_) => match session {
                    FtpSession::Plain(stream) => list_legacy(stream, path).await,
    ...
    async fn set_permissions(&self, _path: &str, _permissions: u32) -> Result<(), AppError> {
        Err(AppError::new(
            ...
            "FTP servers do not provide portable permission changes",
        ))
    }
...
async fn read_chunk<T: suppaftp::tokio::TokioTlsStream + Send>(
    stream: &mut suppaftp::tokio::ImplAsyncFtpStream<T>,
    path: &str,
    offset: u64,
    length: u32,
...
fn parse_mlsx(value: &str, directory: &str) -> Option<FileEntry> {
    ...
    for fact in facts.split(';') {
        ...
        match key.to_ascii_lowercase().as_str() {
            ...
            "modify" => {
                modified_at = NaiveDateTime::parse_from_str(value, "%Y%m%d%H%M%S")
                    .ok()
...
async fn list_legacy<T: suppaftp::tokio::TokioTlsStream + Send>(
    stream: &mut suppaftp::tokio::ImplAsyncFtpStream<T>,
    path: &str,
    ...
    match stream.list(Some(path)).await {
        Ok(lines) => Ok(lines
            .into_iter()
            .filter_map(|line| legacy_list_entry(path, &line))
            .collect()),
        Err(_) => stream
            .nlst(Some(path))
            .await
            .map(|names| {
                names
                    .into_iter()
                    .filter_map(|name| fallback_entry(path, &name))
                    .collect()
            })
            .map_err(map_ftp_error),
    }
...
fn legacy_list_entry(directory: &str, line: &str) -> Option<FileEntry> {
    ...
    Some(FileEntry {
        ...
        modified_at: Some(DateTime::<Utc>::from(file.modified())),
        permissions: None,
        symlink_target: file
            .symlink()
...
fn timed_out() -> AppError {
    AppError::new(ErrorCode::TimedOut, "The FTP connection timed out").retryable()
}
...
#[cfg(test)]
mod tests {
    use siftlane_core::EntryKind;

    use super::{legacy_list_entry, parse_mlsx};

    #[test]
    fn parses_machine_readable_directory_entries() {
    ...
    #[test]
    fn parses_legacy_posix_listings_when_mlsd_is_unavailable() {
        let entry = legacy_list_entry(
            "/uploads",
            "drwxr-xr-x 2 ftp ftp 4096 Jul 21 12:00 releases",
        )
        .unwrap();
        assert_eq!(entry.path, "/uploads/releases");
        assert_eq!(entry.kind, EntryKind::Directory);
    }
}
